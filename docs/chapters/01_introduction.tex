...
Crucially, the sign of the leading spin term reverses between the particle and
antiparticle sectors: for a particle, $H_{\mathrm{spin}} = -\mathbf{b}\cdot
\boldsymbol{\sigma}$, whereas for its antiparticle,
$H_{\mathrm{spin}} = +\mathbf{b}\cdot\boldsymbol{\sigma}$ \cite{KosteleckyLane1999}.
This sign flip is the theoretical linchpin connecting the SME to measurable CPT
asymmetries. The resulting effective operators can be matched onto corresponding
...
= 0$ for every channel. Non-zero values would constitute direct evidence of CPT
violation and would have profound implications for fundamental physics.
Precision antimatter experiments such as BASE and ALPHA at CERN provide some of
the most stringent existing tests of these quantities \cite{Smorra2017,Ahmadi2017}.
