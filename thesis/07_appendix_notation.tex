...
$V_{6,7}$ & $-\tfrac{1}{2mr^2}\big[(\boldsymbol\sigma_1\cdot\mathbf v)(\boldsymbol\sigma_2\cdot\hat{\mathbf r})\pm(\boldsymbol\sigma_1\cdot\hat{\mathbf r})(\boldsymbol\sigma_2\cdot\mathbf v)\big](1-r\,d/dr)\,y(r)$ & $b_0$, $H_{0i}$, $d_{ij}$ \\
...
-- & Master $1/m$ FW expansion & $H'\approx\beta m+\varepsilon_{\text{even}}+\dfrac{\beta\varepsilon_{\text{odd}}^2}{2m}-\dfrac{[\varepsilon_{\text{odd}},\varepsilon_{\text{even}}]}{4m}+O(m^{-2})$ \\
\bottomrule
\end{tabular}
...
\begin{enumerate}
  ...
  \item \textbf{The central empirical limitation.} Of 273 compiled datasets, only 17 touch an antimatter sector, concentrated in the $e$-$\bar p$, $e$-$e^+$, and $e$-$\bar\mu$ channels -- the reason a meaningful matter--antimatter comparison is currently possible in only ten matched pairs.
\end{enumerate}
