...
$b_0$ & Odd & Odd & $+b_0(\boldsymbol\sigma\cdot\mathbf p)/m$ & $V_8$ & $m^{-1}$ & Same caveat as $b_i$ \\
...
$d_{i0}$ & Even & Odd & $+d_{i0}\,m\,\sigma^i$ & $V_2$ & $m^1$ (!) & Same caveat as $H_{ij}$; \textbf{derived and verified} (Sec.~\ref{sec:dmunu}) \\
$d_{ij}$ & Even & Odd & $+d_{ij}\,p^j\,\sigma^i$ & $V_7, V_8$ & $m^0$ (vel.) & Same caveat as $H_{ij}$; structure confirmed, sign open (Sec.~\ref{sec:dmunu}) \\
$d_{00}$ & Even & Odd & $+d_{00}\,(\boldsymbol\sigma\cdot\mathbf p)$ & $V_8$ & $m^0$ (mom.) & Same caveat as $H_{ij}$; structure confirmed, sign open (Sec.~\ref{sec:dmunu}) \\
\bottomrule
\end{tabular}
...
\midrule
$g_Ag_A\cdot V_2\cdot e\bar p$ & Axial-axial & $V_2$ (spin-spin) & $b_\mu$ (CPT-odd) & $0.9998$: consistent with a CPT-odd sign flip, equally consistent with a pure sensitivity gap \\
$g_Ag_A\cdot V_2\cdot ee^+$ & Axial-axial & $V_2$ (spin-spin) & $b_\mu$ (CPT-odd) & $0.9892$: same caveat \\
$g_Ag_A\cdot V_{2+3}\cdot ee^+$ & Axial-axial & $V_{2+3}$ (spin-spin + tensor) & $b_\mu$ (CPT-odd) & $0.9539$: same caveat \\
$g_Ag_A\cdot V_{2+3}\cdot e\bar p$ & Axial-axial & $V_{2+3}$ (spin-spin + tensor) & $b_\mu$ (CPT-odd) & $0.8237$: same caveat \\
$g_Ag_A\cdot V_{2+3}\cdot e\bar p$ & Axial-axial & $V_{2+3}$ (spin-spin + tensor) & $b_\mu$ (CPT-odd) & $0.8044$: same caveat \\
$g_Ag_A\cdot V_2\cdot ee^+$ & Axial-axial & $V_2$ (spin-spin) & $b_\mu$ (CPT-odd) & $0.3336$: same caveat, but not saturated near $1$, so this pair is the one place in the database where the two bounds being compared are of genuinely similar precision \\
\bottomrule
\end{tabular}
\end{table}
