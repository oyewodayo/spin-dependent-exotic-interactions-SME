...
Table~\ref{tab:dm-names} gives a short name for each potential, for quick
reference while reading the closed forms below.
...
Potential & Name \\
\midrule
$V_1$ & Monopole--monopole (spin-independent) \\
$V_2$ & Dipole--dipole (spin--spin) \\
$V_3$ & Tensor dipole--dipole \\
$V_4, V_5$ & Spin--velocity ($+$ gives $V_4$, $-$ gives $V_5$) \\
$V_6, V_7$ & Spin--velocity, symmetric/antisymmetric combination ($+$ gives $V_6$, $-$ gives $V_7$) \\
$V_8$ & Velocity--velocity \\
$V_9, V_{10}$ & Monopole--dipole ($+$ gives $V_9$, $-$ gives $V_{10}$) \\
$V_{11}$ & Spin cross-product (not an $L\cdot S$ coupling) \\
$V_{12}, V_{13}$ & Single-spin--velocity ($+$ gives $V_{12}$, $-$ gives $V_{13}$) \\
$V_{14}$ & Single-spin--velocity \\
$V_{15}, V_{16}$ & Higher-order spin--velocity \\
\bottomrule
\end{tabular}
\caption{Short names for each Dobrescu--Mocioiu potential, keyed to the closed forms in Eqs.~\eqref{eq:v1}--\eqref{eq:v16} below.}
\label{tab:dm-names}
\end{table}
...
not independent structures, with the $+$ sign giving the lower-numbered
potential in each pair ($V_4$, $V_9$, $V_{12}$) and the $-$ sign the
higher-numbered one ($V_5$, $V_{10}$, $V_{13}$). Fourth, $V_6,V_7$ follow the
analogous convention for the symmetric/antisymmetric combination
$(\boldsymbol\sigma_1\cdot\mathbf v)(\boldsymbol\sigma_2\cdot\hat{\mathbf r})
\pm(\boldsymbol\sigma_1\cdot\hat{\mathbf r})(\boldsymbol\sigma_2\cdot\mathbf v)$:
$+$ gives $V_6$, $-$ gives $V_7$.
...
completely unchanged at order $m^0$, and no FW iteration is required for it.
